...
from models import Attendance, AttendanceCheckInResponse, AttendanceCheckOutResponse, AttendanceHistory, AttendanceRequest, Student
...
async def checkin(req: AttendanceRequest, db: Session = Depends(get_session)):
    existing_session = db.exec(select(Attendance).where(
        Attendance.nim == req.nim,
        Attendance.check_out == None
    )).first()

    if existing_session:
        raise HTTPException(status_code=400, detail="Anda belum checkout!") 
...
async def checkout(req: AttendanceRequest, db: Session = Depends(get_session)):
    ...
    if not attendance:
        raise HTTPException(status_code=404, detail="Tidak ada sesi check-in aktif!")
